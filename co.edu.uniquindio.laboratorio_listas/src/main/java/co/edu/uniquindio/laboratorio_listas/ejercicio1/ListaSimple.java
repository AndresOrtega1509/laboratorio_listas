...
//				nodoUltimo = null;
	...
	public ListaSimple<T> obtenerElementosEnPosicionesImpares(ListaSimple<T> lista) {
		ListaSimple<T> nuevaLista = new ListaSimple<>();

		for (int i = 1; i < lista.getTamanio(); i += 2) {
			T valor = obtenerValorNodo(i);
			nuevaLista.agregarfinal(valor);
		}
